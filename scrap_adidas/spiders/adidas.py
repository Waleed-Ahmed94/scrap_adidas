...
class AdidasSpider(scrapy.Spider):
    ...
    def parse_product_pages(self, response):
        ...
        for url in product_urls:
            yield scrapy.Request(url, callback= self.product_info)
    ...
    def product_info(self,response):
        ...
        colors_dup = list()
        for x in colors:
            if x not in colors_dup:
                colors_dup.append(x)
                
        prod_item['variations'] = dict.fromkeys(colors_dup, None)
        #logging.warning('colors %s' % prod_item['variations'].keys())
        ...
        if url is None or data_processed_list == data_list:
            ...
            data_processed_list.append(data_config)
            #color= response.xpath("//div[@class='sibling-product']//li/a[@data-config='{}']/@data-coloroption".format(data_config)).extract_first()
